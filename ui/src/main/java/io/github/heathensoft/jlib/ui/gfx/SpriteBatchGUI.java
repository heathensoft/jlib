...
public class SpriteBatchGUI extends BatchGUI {
    ...
    public static final String SHADER_VERT = "res/jlib/ui/glsl/ui_sprite.vert";
    public static final String SHADER_FRAG = "res/jlib/ui/glsl/ui_sprite.frag";
    ...
    void draw(Rectanglef quad, int abgr, int id, float glow, boolean draw_alpha, boolean ellipse) {
        if (count == buffer_capacity) flush();
        int shader_bits = ((round(clamp(glow) * 63.0f) & 0x3F) << 8);
        shader_bits |= 0xFF;
    ...
    void draw(Texture diffuse, Texture normals, TextureRegion region, Rectanglef quad, int abgr, int id, float glow, boolean draw_alpha, boolean ellipse) {
        if (count == buffer_capacity) flush();
    ...
    void draw(Texture diffuse, Texture normals, Vector4f region, Rectanglef quad, int abgr, int id, float glow, boolean draw_alpha, boolean ellipse) {
        if (count == buffer_capacity) flush();
        // split into diffuse and normals. 15 texture slots (0-14). 15 = NO TEXTURE
        int diffuse_slot = samplersDiffuse.assignSlot(diffuse);
        if (diffuse_slot == 15) { flush();
