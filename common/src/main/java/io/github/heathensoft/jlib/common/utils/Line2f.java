...
public class Line2f {
    ...
    public static float distanceToPoint(float pointAx, float pointAy, float pointBx, float pointBy,
        ...
        if (dot1 > 0) return distance(pointBx, pointBy, pointPx, pointPy);
        float dot2 = -ab0 * ac0 - ab1 * ac1;
        if (dot2 > 0) return distance(pointAx, pointAy, pointPx, pointPy);
    ...
    private int outcode(Rectanglef rect, float x, float y) {
        ...
        if (rect.lengthX() <= 0) {
            ...
            out |= OUT_LEFT;
        } else if (x >= rect.minX + (rect.lengthX())) {
            out |= OUT_RIGHT;
        }
        if (rect.lengthY() <= 0) {
            ...
            out |= OUT_TOP;
        } else if (y >= rect.minY + rect.lengthY()) {
            out |= OUT_BOTTOM;
        }
        return out;
    }
    ...
    public boolean getClipped(Rectanglef rect, Vector2f p0, Vector2f p1) {
        ...
        float maxX = Math.nextDown(rect.minX + rect.lengthX());
        float minY = rect.minY;
        float maxY = Math.nextDown(rect.minY + rect.lengthY());
