...
public abstract class BoxContainer extends Box {
    ...
    protected void render(BoxWindow window, RendererGUI renderer, float x, float y, float dt, int parent_id) {
        ...
        if (this instanceof HBoxContainer container) {for (Box box : contents) {
            ...
            container.processRootInteraction(window,x,y);
    ...
    protected void renderText(BoxWindow window, RendererGUI renderer, float x, float y, float dt) {
        if (this instanceof HBoxContainer container) {
            ...
            for (Box box : contents) {
                if (box.current_width > 0 && box.current_height > 0) {
                    box.renderText(window, renderer, x, y, dt);
                } y -= (box.current_width + inner_spacing);
            }
        } else if (this instanceof RootContainer container) {
            x += container.border_padding;
            y -= container.border_padding;
    ...
    protected void onWindowRegistered(BoxWindow window, BoxContainer parent) {
        for (Box box : contents) {
            box.onWindowRegistered(window,this);
        }
    }
