...
struct CharInfo {           // Helper structure
    ...
    uint sizeBits;  uint fontBits; bool translucent; bool inverted;
};
...
CharInfo getInfoBits(vec3 vertex) {
    ...
    charInfo.glowBits =  (iZ >> 12) & 0x7F; // 7 bit
    charInfo.inverted =  ((iZ >> 13) & 0x01) == 1; // 7 bit
...
Char buildCharacter(vec3 vertex) {
    ...
    if(charInfo.inverted) {
        charStruct.color = invertColor(charStruct.color);
