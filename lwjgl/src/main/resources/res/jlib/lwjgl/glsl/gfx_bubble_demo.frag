...
vec4 draw(vec2 vUv)
{
    ...
    // ring
    n0 = snoise3( vec3(uv * noiseScale, u_time * 0.5) ) * 0.5 + 0.5;
    r0 = mix(mix(innerRadius, 1.0, 0.4), mix(innerRadius, 1.0, 0.6), n0);
    d0 = distance(uv, r0 / len * uv);
    v0 = light1(1.0, 10.0, d0);
    v0 *= smoothstep(r0 * 1.05, r0, len);
    cl = cos(ang + u_time * 2.0) * 0.5 + 0.5;
...
void main() {
    vec2 uv_coord = vs_in.uv;
    if(uv_coord.x == 0.0) uv_coord.x = 0.0001;
    if(uv_coord.y == 0.0) uv_coord.y = 0.0001;
    vec4 col = draw(uv_coord);
    vec3 bg = vec3(0.0,0.01,0.1);
    f_diffuse = mix(bg, col.rgb, col.a); //normal blend
}
