...
import static io.github.heathensoft.jlib.common.utils.U.abs;
...
public class BitmapFont extends AtlasData {

    public BitmapFont(Bitmap bitmap, String info) { super(bitmap, info); }

    public BitmapFont(Bitmap bitmap, String info, String name) { super(bitmap, info, name); }

    public static BitmapFont create(String name, ByteBuffer ttf, int size, int spacing, float protrusion) throws Exception { return create(name, ttf, size, spacing, protrusion, 0); }

    public static BitmapFont create(String name, ByteBuffer ttf, int size, int spacing, float protrusion, float noise) throws Exception { return create(name, ttf, size, spacing, protrusion, noise, GL_LINEAR, GL_LINEAR); }

    public static BitmapFont create(String name, ByteBuffer ttf, int size, int spacing, float protrusion, float noise, int minFilter, int magFilter) throws Exception { return create(name, ttf, size, spacing, protrusion, noise,minFilter, magFilter, false); }
    ...
    public static BitmapFont create(String name, ByteBuffer ttf, int size, int spacing, float protrusion, float noise, int minFilter, int magFilter, boolean mipMap) throws Exception {
        ...
        return new BitmapFont(bitmap,builder.toString(),name);
    }

    public static BitmapFont create(String name, ByteBuffer ttf, int size, int spacing, int lineGapAdjust, float protrusion, float noise, int minFilter, int magFilter, boolean mipMap) throws Exception {
        name = name == null || name.isBlank() ? "UnnamedFont" : name;
        name = name.trim();
        size = Math.max(1,size);
        if (abs(lineGapAdjust) > (size / 2f)) {
            lineGapAdjust = (int)((lineGapAdjust < 0) ? - (size / 2f) : (size / 2f));
        } spacing = Math.max(0,spacing);
        noise = clamp(noise);
        protrusion = clamp(protrusion,-1.0f,1.0f);
        STBTTFontinfo info = STBTTFontinfo.create();
        ...
        builder.append("F ");
        builder.append(name).append(' ');
        builder.append(size + lineGapAdjust).append(' ');
        builder.append(num_glyphs).append(' ');
        builder.append(image_width).append(' ');
        builder.append(image_height).append(' ');
        builder.append(protrusion).append(' ');
        ...
        builder.append(ascent).append(' ');
        builder.append(descent).append(' ');
        builder.append(lineGap + lineGapAdjust).append(' ');
        builder.append(monospaced ? 1 : 0).append('\n');
        builder.append("T ");
        builder.append(minFilter).append(' ');
        builder.append(magFilter).append(' ');
